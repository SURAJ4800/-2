  ...
  previewNavbar.innerHTML = `
        <div class="container flex text-center jc-ai-center">
            <div class="site-title flex align-item-center">
                <div>
                    <a href="index.html"><h1><span class="clr-red">Karim City</span> <span class="clr-green">Canteen</span></h1></a>
                </div>
...
// Best Dishes Section
var bestDishes = [
  { title: "Masala Sandwich", rating: 4, imgURL: "assets/images/s2.jpg", description: "Sumptuous delicious food baked in our cafe", price: 30 },
  { title: "Red Pasta", rating: 5, imgURL: "assets/images/pasta.jpg", description: "Sumptuous delicious food baked in our cafe", price: 45 },
  { title: "White Pasta", rating: 5, imgURL: "assets/images/white-pasta.jpg", description: "Sumptuous delicious food baked in our cafe", price: 25 },
  { title: "French Fries", rating: 4, imgURL: "assets/images/shezuan.jpg", description: "Sumptuous delicious food baked in our cafe", price: 35 },
];
...
signInModalbtn.forEach((element) => {
  element.addEventListener("click", () => { signModal.classList.toggle("show"); return signIn_UI; });
});
signUpModalbtn.forEach((element) => {
  element.addEventListener("click", () => { signUpModal.classList.toggle("show"); return signUp_UI; });
});
cartBtn.forEach((element) => {
  element.addEventListener("click", () => { cartOverlay.classList.add("show"); cartInnerlay.classList.add("showCart"); });
});

window.onclick = function (event) {
  if (event.target == signModal || event.target == signUpModal) { signModal.classList.remove("show"); signUpModal.classList.remove("show"); }
  if (event.target == cartOverlay) { cartOverlay.classList.remove("show"); cartInnerlay.classList.remove("showCart"); }
};
  ...
  signUp_UI = signUpModal.innerHTML = `
    <div class="modal-container">
        <div class="modal-content">
            <div class="md">Welcome to <br><span class="clr-red md">Karim City</span> <span class="clr-green md">Canteen</span></div>
        ...
        <label class="text-center"><input type="checkbox" checked="checked" name="remember"> Remember me</label>
        <div class="modal-content m-content-2 py-1">
            Click “Register” to agree to Terms of Service. Have account? 
            <a id="sign-modal" onclick="signUpModal.classList.remove('show'); signModal.classList.add('show');" style="cursor: pointer;">
...
if (signModal) {
  signIn_UI = signModal.innerHTML = `
    <div class="modal-container">
        ...
        <label class="text-center"><input type="checkbox" checked="checked" name="remember"> Remember me</label>
        <div class="modal-content m-content-2 py-1">
            Click “Sign In” to agree to Terms of Service. No account? 
            <a id="signup-modal" onclick="signModal.classList.remove('show'); signUpModal.classList.add('show');" style="cursor: pointer;">
...
// --------------------------------------------------------------------------------
// KARIM CITY FOOTER LOGIC (APPLIES TO HOME AND CLIENT PAGES)
// --------------------------------------------------------------------------------
      ...
      element.innerHTML = `
            <section class="container-min">
                <main class="grid grid-3">
                    <article class="about">
                        <h1 class="sm">About Karim City</h1>
                        <p>Karim City College Sakchi, Jamshedpur - 831001, Jharkhand, INDIA. Serving fresh food to students and faculty.</p>
                        <ul class="py-1">
                            <li><i class="fab fa-facebook-f"></i><a href="https://www.facebook.com/kccjsrofficial/" target="_blank"> Facebook</a></li>
                            <li><i class="fab fa-instagram"></i><a href="https://www.instagram.com/karim_city_college_jamshedpur/?hl=en" target="_blank">Instagram</a></li>
                        </ul>
                    </article>
                    ...
                    <article class="contact">
                        <ul>
                            ...
                            <li> <i class="fas fa-phone-alt"></i> <p>+91 657-2431778</p></li>
                            <li> <i class="fas fa-phone"></i> <p>0657-2423863, 2440206</p></li>
                            <li> <i class="fas fa-envelope"></i> <p>office@karimcitycollege.org</p></li>
                            <li> <i class="fas fa-map-marker-alt"></i> <p>Sakchi, Jamshedpur - 831001</p></li>
                        </ul>
                    </article>
                </main>
                <main class="deep-devs flex">
                    <div>© Copyright 2026. All Rights Reserved</div>
                    <div class="">Designed & Developed by <span class="clr-red" style="font-weight:bold;">Amigos</span></div>
                </main>
            </section>
        `;
    });
...
// Authenticated User Footer (Reusing the same Karim City logic for consistency)
            ...
            <section class="container-min">
                <main class="grid grid-3">
                    <article class="about">
                        ...
                        <p>Karim City College Sakchi, Jamshedpur - 831001, Jharkhand, INDIA.</p>
                        <ul class="py-1">
                            <li><i class="fab fa-facebook-f"></i><a href="https://www.facebook.com/kccjsrofficial/" target="_blank"> Facebook</a></li>
                            <li><i class="fab fa-instagram"></i><a href="https://www.instagram.com/karim_city_college_jamshedpur/?hl=en" target="_blank">Instagram</a></li>
                        </ul>
...
function setOrderDetails(trimedEmailID) {
  firebase.database().ref("Users_Order/" + trimedEmailID + "_Orders").on("value", function (snapshot) {
      if (snapshot.exists()) {
        ...
        userOrder.forEach((key) => {
          ...
          var trimedID = "UO" + date.replace(/\//g, "");

          if (data[key].Order_Status === true) {
            i += 1;
            if (orderStatus === true) { orderStatus = "Cooking"; } else { orderStatus = "Not Available"; }
            ...
            div.innerHTML = `
                        <table class="main-details">
                            ...
                            <thead>
                                <tr>
                                    <th><i class="fas fa-th-list"></i> Order ID</th>
                                    <th><i class="fas fa-tenge"></i> Total</th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td data-label="Status">${orderStatus}</td>
                                </tr>
                            </tbody>
                        </table>
                    `;
